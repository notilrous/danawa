...
def load_first_category():
    res_base = requests.get(base_URL, timeout=5, headers=headers, verify=False)
...
def load_second_category(code_num):
    res_base = requests.get(base_URL, timeout=5, headers=headers, verify=False)
    res_URL = requests.get(URL + code_num, timeout=5, headers=headers, verify=False)
    if res_base.status_code == 200:
        # 두번째 카테고리
...
def load_prod(link):
    ...
    res = requests.get(final_URL + str(slice_num[0]), timeout=5, headers=headers, verify=False)
